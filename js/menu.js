import { guardarPedido, obtenerPagoMovil, obtenerProductos } from "./firebase.js";
import { obtenerTasaBCV, getTasaActual, convertirABolivares, formatBs } from "./bcv.js";
import { MENU_DEFAULT, WHATSAPP } from "./data.js";
    ...
    <div class="item-card ${sel ? "selected" : ""}">
      ...
      <div class="item-info">
        <div class="item-name-row">
          ...
          <div class="item-price ${sel ? "sel" : ""}">$${parseFloat(item.price).toFixed(2)}</div>
        </div>
...
function updateSummary() {
  ...
  const total = items.reduce((s, i) => s + parseFloat(i.price) * qty[i.id], 0);
  ...
  document.getElementById("summary-items").innerHTML = items.map(i =>
    `<div class="summary-row"><span>${i.name} ×${qty[i.id]}</span><span>$${(parseFloat(i.price)*qty[i.id]).toFixed(2)}</span></div>`
  ).join("");
  ...
  if (tasa) { document.getElementById("total-bs").textContent = formatBs(convertirABolivares(total)); bsEl.style.display = "flex"; }
